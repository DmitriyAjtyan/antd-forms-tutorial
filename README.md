...
Задание 17.
<br>
Теперь, когда наша форма регистрации, наконец-то, работает правильно настало время заняться рефакторингом кода, который мы тут понаписали: вынести константы, утилиты, типы и хуки в отдельные места, разнести код на компоненты, вынести дублирующийся код в какие-то переиспользуемые функции и т.д. В общем, последним заданием по этой форме будет рефакторинг. Смотрите ничего не сломайте в ходе вашего рефакторинга...
<br>
Подумайте хорошенько головой, как после рефакторинга убедиться в том, что вы ничего не сломали. Протестить все 16 заданий заново руками? А если в ходе code review выяснится, что код можно сделать ещё лучше, то после очередных изменений в нём снова будете тестить руками все 16 задач?
